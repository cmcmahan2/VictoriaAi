...
// What the buy-box parser returns — a structured criteria object the UI can
// turn into a search query + filter state.
...
function fallbackParse(text: string): ParsedBuyBox {
  ...
  if (over && !/bed|bd|bath/.test(over[0])) minPrice = parseMoney(over[1]);
  ...
  // Market: try "in <City, ST>" or match a known market
  ...
  if (propertyTypes.length) parts.push(propertyTypes.join('/'));
  ...
  return {
    ...
    maxDaysOnMarket: null, requireDistressSignals, minScore,
    summary: parts.join(' '),
  };
}
...
export async function POST(req: Request) {
  try {
    ...
    if (!env.ANTHROPIC_API_KEY) {
      return NextResponse.json({ ok: true, parsed: fallbackParse(text), usedAi: false });
    }
    ...
    let parsed: ParsedBuyBox;
    try {
      parsed = sanitize(JSON.parse(match[0]) as Partial<ParsedBuyBox>);
    } catch {
      parsed = fallbackParse(text);
      return NextResponse.json({ ok: true, parsed, usedAi: false });
    }

    return NextResponse.json({ ok: true, parsed, usedAi: true });
  } catch (err) {
