...
- `wss://ws-live-data.polymarket.com` (no auth) relays that exact feed: topic `crypto_prices_chainlink` (symbols like `btc/usd`) = resolution feed / strike capture; topic `crypto_prices` (symbols like `btcusdt`) = Binance-sourced leading feed. Subscribe **without a `filters` field**: `{"action":"subscribe","subscriptions":[{"topic":"crypto_prices_chainlink","type":"*"},{"topic":"crypto_prices","type":"*"}]}`. Each topic then streams per-second `type:"update"` ticks — `{topic,type:"update",timestamp,payload:{symbol,value,full_accuracy_value}}` — for **all** symbols on that topic; filter to BTC client-side. PING every 5s. **[LIVE-VERIFIED 2026-07-08; collector `scripts/collect_rtds.py` runs locally — host firewalled in the web sandbox]**
  - ⚠️ **FALSIFIED [LIVE 2026-07-08]:** the earlier `[DOC]` claim that a `filters` JSON-string (`"filters":"{\"symbol\":\"btc/usd\"}"`) was required is **wrong and load-bearing** — sending `filters` *suppresses the stream*: the server returns at most a one-time snapshot (`type:"subscribe"`, a ~2-min backfill array) and then goes silent, no updates. This is exactly why RTDS logged ~0 bytes of ticks for days. Omit `filters` entirely. (`crypto_prices_chainlink` is where `btc/usd` lives; a filterless `crypto_prices` sub carries `btcusdt` but *not* `btc/usd`, so subscribe to both topics.)
